...
impl Pattern {
        ...
        PF: Fn(&Path) -> bool,
    {
        ...
        // The Windows-only runtime accepts both slash-rooted shell paths and native
        // drive-letter roots via `sys::fs::pattern_path_root`.
        ...
        for piece in &self.pieces {
            match piece {
                ...
                PatternPiece::Literal(s) => {
                    for c in s.chars() {
                        if crate::engine::regex::regex_char_is_special(c) {
...
fn pattern_to_regex_str(
    ...
    Ok(crate::parser::pattern::pattern_to_regex_str(
        pattern,
...
mod tests {
    ...
    /// Regression test for the prefix-strip mismatch fix.
    ///
    /// Previously, `expand` built `prefix_to_remove` with native `\` separators
    /// while `push_path_for_pattern` appended components with `/`. The mismatch
    /// made `strip_prefix` a no-op, so relative globs produced absolute paths.
    /// This test verifies results are returned relative to the working directory.
    #[test]
    ...
    fn test_absolute_glob_returns_absolute_paths() -> Result<()> {
        ...
        // Normalize to forward slashes; `pattern_path_root` accepts both slash
        // rooted shell paths and native drive-letter roots.
