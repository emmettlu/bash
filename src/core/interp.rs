...
use crate::core::arithmetic::{self, ExpandAndEvaluate};
use crate::core::commands::{self, CommandArg};
use crate::core::env::{EnvironmentLookup, EnvironmentScope, valid_variable_name};
use crate::core::openfiles::{OpenFile, OpenFiles};
...
impl Execute for ast::Pipeline {
    async fn execute(
        ...
        if !result.is_success()
            ...
            && !self.bang
            && shell.traps().handles(crate::core::traps::TrapSignal::Err)
        {
            shell
                .invoke_trap_handler(crate::core::traps::TrapSignal::Err, &params)
                .await?;
        }
...
impl Execute for ast::FunctionDefinition {
    async fn execute(
        ...
        let source_info = shell
            .call_stack()
            .current_frame()
            .map_or_else(crate::core::SourceInfo::default, |frame| {
...
async fn execute_command(
    ...
    // returning.
    let mut guard =
        crate::core::env::ScopeGuard::new(&mut context.shell, EnvironmentScope::Command);
