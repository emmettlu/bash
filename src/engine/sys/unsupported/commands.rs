...
use crate::engine::{ShellFd, error, openfiles};

/// Extension trait for command features that are not natively supported by this runtime.
...
/// Extension trait for exit status features that are not natively supported by this runtime.
...
impl CommandFdInjectionExt for std::process::Command {
    fn inject_fds(
        ...
        for (fd, file) in open_files {
            match fd {
                openfiles::OpenFiles::STDIN_FD => {
                    self.stdin(file);
                }
                openfiles::OpenFiles::STDOUT_FD => {
                    self.stdout(file);
                }
                openfiles::OpenFiles::STDERR_FD => {
                    self.stderr(file);
                }
                _ => {
                    return Err(
                        error::ErrorKind::NotSupported("non-standard fd redirections").into(),
                    );
                }
            }
        }
