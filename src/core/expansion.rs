...
use crate::core::ExecutionParameters;
use crate::core::arithmetic;
use crate::core::arithmetic::ExpandAndEvaluate;
use crate::core::braceexpansion;
...
use crate::core::sys;
use crate::core::trace_categories;
use crate::core::variables::ShellValueUnsetType;
use crate::core::variables::ShellVariable;
use crate::core::variables::{self, ShellValue};
...
impl From<ExpansionPiece> for crate::core::regex::RegexPiece {
    fn from(piece: ExpansionPiece) -> Self {
        match piece {
            ExpansionPiece::Unsplittable(s) => Self::Literal(s),
            ExpansionPiece::Splittable(s) => Self::Pattern(s),
...
pub(crate) async fn basic_expand_regex(
    ...
    params: &ExecutionParameters,
    word_str: impl AsRef<str>,
) -> Result<crate::core::regex::Regex, error::Error> {
    let mut expander = WordExpander::new(shell, params);
...
impl<'a, SE: extensions::ShellExtensions> WordExpander<'a, SE> {
        ...
        let regex_pieces: Vec<_> = expansion
            ...
            .map(|field| {
                field
                    .0
                    .into_iter()
                    .map(crate::core::regex::RegexPiece::from)
                    .collect::<Vec<_>>()
            })
            .intersperse(vec![crate::core::regex::RegexPiece::Literal(String::from(
                " ",
            ))])
            .flatten()
            .collect();

        Ok(crate::core::regex::Regex::from(regex_pieces)
    ...
    async fn basic_expand(&mut self, word: &str) -> Result<Expansion, error::Error> {
        ...
        let pieces = if self.heredoc_mode {
            ...
            crate::parser::word::parse_heredoc(brace_expanded.as_ref(), &self.parser_options)?
        } else {
            crate::parser::word::parse(brace_expanded.as_ref(), &self.parser_options)?
        };
...
mod tests {
    ...
    async fn test_full_expansion() -> Result<()> {
        let mut shell = crate::core::shell::Shell::builder().build().await?;
