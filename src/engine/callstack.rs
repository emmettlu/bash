...
use crate::engine::{functions, traps};
use std::{
    borrow::Cow,
...
impl Frame {
    ...
    pub fn adjusted_source_info(&self) -> crate::engine::SourceInfo {
    ...
    pub fn current_pos_as_source_info(&self) -> crate::engine::SourceInfo {
...
impl CallStack {
    ...
    pub fn current_pos_as_source_info(&self) -> crate::engine::SourceInfo {
        let Some(frame) = self.frames.front() else {
            return crate::engine::SourceInfo::default();
        };
    ...
    pub fn push_trap_handler(
        ...
        let source_info = handler.map_or_else(crate::engine::SourceInfo::default, |h| {
            h.source_info.clone()
        });
    ...
    pub fn push_eval(&mut self) {
        self.frames.push_front(Frame {
            ...
            args: vec![],
            source_info: crate::engine::SourceInfo::from("eval"), // TODO(source-info): fill this out
            current_line_offset: 0,
            current: None, // TODO(source-info): fill this out
            entry: None,   // TODO(source-info): fill this out
        });
    ...
    pub fn push_command_string(&mut self) {
        self.frames.push_front(Frame {
            ...
            args: vec![],
            source_info: crate::engine::SourceInfo::from("environment"),
            current_line_offset: 0,
            current: None, // TODO(source-info): fill this out
            entry: None,   // TODO(source-info): fill this out
        });
    ...
    pub fn push_interactive_session(&mut self) {
        self.frames.push_front(Frame {
            ...
            args: vec![],
            current_line_offset: 0,
            source_info: crate::engine::SourceInfo::from("main"),
            current: None, // TODO(source-info): fill this out
            entry: None,   // TODO(source-info): fill this out
        });
    }
