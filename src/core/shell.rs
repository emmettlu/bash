...
pub struct Shell<SE: extensions::ShellExtensions = extensions::DefaultShellExtensions> {
    ...
    traps: crate::core::traps::TrapHandlerConfig,
    ...
    call_stack: crate::core::callstack::CallStack,
    ...
    history: Option<crate::core::history::History>,
}
    ...
    pub(crate) fn new(options: CreateOptions<SE>) -> Result<Self, error::Error> {
        ...
        if shell.options.enable_command_history {
            shell.history = shell
                .load_history()
                .unwrap_or_default()
                .or_else(|| Some(crate::core::history::History::default()));
        }
...
impl<SE: extensions::ShellExtensions> Shell<SE> {
    ...
    pub(crate) fn update_last_arg_variable(&mut self, last_arg: Option<String>) {
        ...
        if self
            .env
            .get_using_policy("_", crate::core::env::EnvironmentLookup::Anywhere)
        ...
        let _ = self
            .env
            .set_global("_", crate::core::variables::ShellVariable::new(value));
    }
...
impl<SE: extensions::ShellExtensions> ShellState for Shell<SE> {
    ...
    pub fn traps(&self) -> &crate::core::traps::TrapHandlerConfig {
        &self.traps
    }

    /// Returns a mutable reference to the shell's trap handler configuration.
    pub fn traps_mut(&mut self) -> &mut crate::core::traps::TrapHandlerConfig {
        &mut self.traps
    }
    ...
    pub fn call_stack(&self) -> &crate::core::callstack::CallStack {
        &self.call_stack
    }

    /// Returns the shell's history, if it exists.
    pub fn history(&self) -> Option<&crate::core::history::History> {
        self.history.as_ref()
    }

    /// Returns a mutable reference to the shell's history, if it exists.
    pub fn history_mut(&mut self) -> Option<&mut crate::core::history::History> {
        self.history.as_mut()
    }
