//! Unsupported filesystem compatibility helpers.

use crate::engine::error;

/// Converts a `Path` to a shell-display string.
...
/// Returns default search paths for standard utilities.
///
/// The Windows-only runtime does not currently define extra fallback paths.
...
///
/// The Windows-only runtime overrides this when a native null sink is available.
pub fn open_null_file() -> Result<std::fs::File, error::Error> {
    Err(error::ErrorKind::NotSupported("opening null file").into())
}

/// Handles shell special file paths that do not exist as native Windows paths.
//
// Unsupported fallback returns no result.
...
///
/// Unsupported fallback returns `None`.
...
///
/// Unsupported fallback returns `false`.
...
///
/// Unsupported fallback only treats `/` as a path separator.
