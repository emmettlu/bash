...
use crate::engine::{Shell, ShellFd, extensions, results, sys};
...
pub enum ErrorKind {
    ...
    UnexpectedScopeType {
        ...
        expected: crate::engine::env::EnvironmentScope,
        /// The actual scope type.
        actual: crate::engine::env::EnvironmentScope,
    },
    ...
    EvalError(#[from] crate::engine::arithmetic::EvalError),
    ...
    #[error("{1}: {0}")]
    ParseError(crate::engine::parser::ParseError, crate::engine::SourceInfo),

    /// An error occurred while parsing a function body.
    #[error("{0}: {1}")]
    FunctionParseError(String, crate::engine::parser::ParseError),

    /// An error occurred while parsing a word.
    #[error(transparent)]
    WordParseError(#[from] crate::engine::parser::WordParseError),

    /// Unable to parse a test command.
    #[error("invalid test command")]
    TestCommandParseError(#[from] crate::engine::parser::TestCommandParseError),

    /// Unable to parse a key binding specification.
    #[error(transparent)]
    BindingParseError(#[from] crate::engine::parser::BindingParseError),
    ...
    /// A system integration error occurred.
    #[error("system error: {0}")]
    SystemError(#[from] sys::SystemError),
    ...
    /// Operation not supported by this Windows-only runtime.
